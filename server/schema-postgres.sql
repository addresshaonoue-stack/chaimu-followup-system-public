...
CREATE TABLE IF NOT EXISTS followups (
  id SERIAL PRIMARY KEY,
  patient_id INTEGER NOT NULL REFERENCES patients(id) ON DELETE CASCADE,
  schedule_id INTEGER REFERENCES followup_schedules(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS clinician_evaluations (
  ...
  clinician_effect TEXT NOT NULL DEFAULT '暂未评价' CHECK (clinician_effect IN ('明显改善', '部分改善', '暂未见明显改善', '需进一步评估', '暂未评价')),
  clinician_safety TEXT NOT NULL DEFAULT '未记录明显不良事件' CHECK (clinician_safety IN ('未记录明显不良事件', '轻度不良事件', '中度不良事件', '重度不良事件')),
  clinician_note TEXT,
  evaluator_doctor INTEGER NOT NULL REFERENCES users(id),
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_patients_research_id ON patients(research_id);
CREATE INDEX IF NOT EXISTS idx_patients_token ON patients(token);
CREATE INDEX IF NOT EXISTS idx_patients_token_disabled ON patients(token_disabled);
CREATE INDEX IF NOT EXISTS idx_schedules_patient_due ON followup_schedules(patient_id, due_date);
CREATE INDEX IF NOT EXISTS idx_followups_patient_date ON followups(patient_id, visit_date);
CREATE INDEX IF NOT EXISTS idx_followups_schedule ON followups(schedule_id);
CREATE INDEX IF NOT EXISTS idx_followups_adverse ON followups(has_adverse_reaction, severity);
CREATE INDEX IF NOT EXISTS idx_evaluations_patient_date ON clinician_evaluations(patient_id, evaluation_date);
CREATE INDEX IF NOT EXISTS idx_audit_logs_created_at ON audit_logs(created_at);
CREATE INDEX IF NOT EXISTS idx_audit_logs_action ON audit_logs(action_type, object_type);
